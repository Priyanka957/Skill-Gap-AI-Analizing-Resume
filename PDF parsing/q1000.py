...
def looks_like_resume(text):
    ...
    return sum(1 for kw in resume_keywords if kw in text_lower) >= 2
...
def validate_and_extract(file, file_label):
    # 1️⃣ No file uploaded
    if file is None:
        return None, f"Please upload {file_label}."

    # 2️⃣ Wrong file format
    ext = file.name.split(".")[-1].lower()
    if ext not in ALLOWED_EXTENSIONS:
        return None, (
            f"Unsupported file format for {file_label}. "
            "Please upload PDF, DOCX, or TXT."
        )

    # 3️⃣ Extraction failure
    try:
        text = extract_text(file)
        text = clean_text(text)
    except Exception:
        return None, (
            f"Unable to read {file_label}. "
            "Please upload a valid document."
        )

    # 4️⃣ Empty or unreadable file
    if not text or len(text.strip()) == 0:
        return None, (
            f"Unable to extract text from the {file_label}. "
            "The file may be empty or scanned."
        )

    # 5️⃣ Content too short
    if len(text.strip()) < 200:
        return None, (
            f"The uploaded {file_label} doesn’t seem to contain "
            "enough content for analysis."
        )
...
# 4️⃣ ANALYZE BUTTON (ADD YOUR BLOCK HERE ⬇️)
if st.button("Analyze"):
    resume_text, resume_error = validate_and_extract(resume_file, "Resume")
    jd_text, jd_error = validate_and_extract(jd_file, "Job Description")
    ...
    # 🔽 NEXT: skill extraction, charts, matching logic
...
if resume_text is None:
    st.error(
        "❌ Invalid resume file.\n\n"
        "Please upload a valid Resume (PDF, DOCX, or TXT) "
        "containing sections like Skills, Experience, or Education."
    )
    st.stop()
...
# ---------- Minimum Content Warnings (NON-BLOCKING) ----------
if len(resume_text.strip()) < 50:
    st.warning(
        "⚠️ Resume appears very short. "
        "Minimum 50 characters recommended for proper analysis."
    )

if len(jd_text.strip()) < 50:
    st.warning(
        "⚠️ Job Description appears very short. "
        "Minimum 50 characters recommended for proper analysis."
    )
...
with col3:
    ...
    st.metric("Basic Matches", moderate_matches)
with col4:
...
# Create the horizontal bar chart
  ...
  # ================= NEW INNOVATIVE FEATURE =================
        ...
        st.markdown('<div class="priority-title">⚖️ Skill Priority Radar</div>', unsafe_allow_html=True)
